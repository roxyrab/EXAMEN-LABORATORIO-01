...
void AAventuraUSFX022026GameMode::BeginPlay()
{
	...
	for (int i = 0; i < 50; i++) {
		...
		case 0:
			plataformaActual = World->SpawnActor<APlataformaAereo>(APlataformaAereo::StaticClass(), SpawnLocation, Rotacion);

			//tipoPlataformaActual = ETipoPlataforma::PLATAFORMA_AEREA;
			break;
		case 1:
			plataformaActual = World->SpawnActor<APlataformaTerrestre>(APlataformaTerrestre::StaticClass(), SpawnLocation, Rotacion);
			//tipoPlataformaActual = ETipoPlataforma::PLATAFORMA_TERRESTRE;
			break;
		case 2:
			plataformaActual = World->SpawnActor<APlataformaSubterranea>(APlataformaSubterranea::StaticClass(), SpawnLocation, Rotacion);
			//tipoPlataformaActual = ETipoPlataforma::PLATAFORMA_SUBTERRANEA;
			break;
		case 3:
			plataformaActual = World->SpawnActor<APlataformaAcuatica>(APlataformaAcuatica::StaticClass(), SpawnLocation, Rotacion);
	...
	*/
	GetWorldTimerManager().SetTimer(TimerEliminarPlataforma, this, &AAventuraUSFX022026GameMode::EliminarPlataforma, 1.0f, true);

}

void AAventuraUSFX022026GameMode::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);
}

void AAventuraUSFX022026GameMode::EliminarPlataforma()
{
		...
		if (aPlataformas[i] != nullptr)
		{
			...
			if (plataformaActual != nullptr)
			{
				if (plataformaActual->tipoPlataforma == static_cast<ETipoPlataforma>(tipoPlataformaActual))
				{
					...
					// Lo eliminamos tambi?n del TArray
					aPlataformas.RemoveAt(i);

					// Salimos del bucle despu?s de eliminar el primero
					break;
				}
			}
		}
